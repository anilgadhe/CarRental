...
import { dummyUserData , ownerMenuLinks } from '../../assets/data';
   ...
   const {user,axios , fetchUser}=useAppContext();
    ...
    const updateImage = async () => {
      try {
        ...
        const {data}= await axios.post('/owner/v1/update-image',formData)
    ...
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
            ...
            <div className='group relative'>
                ...
                <label htmlFor='image'>
                    <img src={image ? URL.createObjectURL(image) : user?.image || "https://images.pexels.com/photos/924824/pexels-photo-924824.jpeg"} alt="image" className='h-10 md:h-14 w-14 rounded-full mx-auto'/>
            ...
            {image && (
                ...
                text-primary cursor-pointer' onClick={updateImage}>Save <img src='/check_icon.svg' alt='check' width={13}/></button>
            )}
            ...
            <div className='w-full'>
                 {ownerMenuLinks.map((link,index)=>(
                    <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
                        <img src={link.path === location.pathname ? link.coloredIcon : link.icon } alt='car_icon'/>
